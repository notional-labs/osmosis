...
edit_genesis () {
    ...
    dasel put -r json -t string -f $GENESIS '.app_state.staking.params.bond_denom' -v 'uosmo'
    dasel put -r json -t string -f $GENESIS '.app_state.staking.params.unbonding_time' -v '240s'

    # Update crisis module
    dasel put -r json -t string -f $GENESIS '.app_state.crisis.constant_fee.denom' -v  'uosmo'

    # Update gov module
    dasel put -r json -t string -f $GENESIS '.app_state.gov.voting_params.voting_period' -v  '60s'
    dasel put -r json -t string -f $GENESIS '.app_state.gov.params.min_deposit.[0].denom' -v  'uosmo'

    # Update epochs module
    dasel put -r json -t string -f $GENESIS '.app_state.epochs.epochs.[1].duration' -v  "60s"

    # Update poolincentives module
    dasel put -r json -t string -f $GENESIS '.app_state.poolincentives.lockable_durations.[0]' -v  "120s"
    dasel put -r json -t string -f $GENESIS '.app_state.poolincentives.lockable_durations.[1]' -v  "180s"
    dasel put -r json -t string -f $GENESIS '.app_state.poolincentives.lockable_durations.[2]' -v  "240s"
    dasel put -r json -t string -f $GENESIS '.app_state.poolincentives.params.minted_denom' -v  "uosmo"

    # Update incentives module
    dasel put -r json -t string -f $GENESIS '.app_state.incentives.lockable_durations.[0]' -v  "1s"
    dasel put -r json -t string -f $GENESIS '.app_state.incentives.lockable_durations.[1]' -v  "120s"
    dasel put -r json -t string -f $GENESIS '.app_state.incentives.lockable_durations.[2]' -v  "180s"
    dasel put -r json -t string -f $GENESIS '.app_state.incentives.lockable_durations.[3]' -v  "240s"
    dasel put -r json -t string -f $GENESIS '.app_state.incentives.params.distr_epoch_identifier' -v  "day"

    # Update mint module
    dasel put -r json -t string -f $GENESIS '.app_state.mint.params.mint_denom' -v  "uosmo"
    dasel put -r json -t string -f $GENESIS '.app_state.mint.params.epoch_identifier' -v  "day"

    # Update gamm module
    dasel put -r json -t string -f $GENESIS '.app_state.gamm.params.pool_creation_fee.[0].denom' -v  "uosmo"

    # Update txfee basedenom
    dasel put -r json -t string -f $GENESIS '.app_state.txfees.basedenom' -v  "uosmo"

    # Update wasm permission (Nobody or Everybody)
    dasel put -r json -t string -f $GENESIS '.app_state.wasm.params.code_upload_access.permission' -v  "Everybody"
}
...
edit_config () {
    # Remove seeds
    dasel put -r toml -t string -f $CONFIG_FOLDER/config.toml '.p2p.seeds'  -v  ''

    # Expose the rpc
    dasel put -r toml -t string -f $CONFIG_FOLDER/config.toml '.rpc.laddr' -v  "tcp://0.0.0.0:26657"
}
